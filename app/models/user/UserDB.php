    ...
    public static function getSearchUsers($select, $search, $options) {
        try {
            ...
            if ($select["membershipYear"]) {
                $selectPart .= ", members_user_card_year.membership_year";
            ...
            if ($select["nCheckInsPerYear"]) {
                $selectPart .= ", COUNT(*) n_check_ins_per_year";
            ...
            if (!$stmt->execute())
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($select["nCheckInsByYear"])
                        $searchUsers[$i]["nCheckInsPerYear"] = $row["n_check_ins_per_year"];
                    if ($select["cardNumber"])
