    ...
    public static function getNCheckInsThisYearByUserId($userId){
        try {
            ...
            if ($stmt) {
                ...
                $stmt->bind_param('i', $userId);
                if (!$stmt->execute())
                    throw new CheckInDBException('Unknown error during statement execution while getting the user\'s check-ins.', CheckInDBException::UNKNOWNERROR);
                else {
                    $stmt->bind_result($nCheckIns);
                    if ($stmt->fetch())
                        return $nCheckIns;
    ...
    public static function getTotalCheckInsByUserId($userId){
        try {
            ...
            if ($stmt) {
                ...
                $stmt->bind_param('i', $userId);
                if (!$stmt->execute())
                    throw new CheckInDBException('Unknown error during statement execution while counting the user\'s check-ins.', CheckInDBException::UNKNOWNERROR);
                else {
                    $stmt->bind_result($nCheckIns);
                    if ($stmt->fetch())
                        return $nCheckIns;
